...
export default function CreateListing() {
  ...
  const {
    getRootProps: getListingImageRootProps,
    getInputProps: getListingImageInputProps,
  } = useDropzone({
  ...
  function handleChange(event) {
    ...
    // Check if the field is "price" and parse it as a number
    const newValue = id === "price" ? parseFloat(value) : value;

    setListing({ ...listing, [id]: newValue });
    console.log(listing);
  }
  ...
  return (
    <form style={{ padding: "40px", display: "flex", flexDirection: "column" }}>
      <h1>New Listing</h1>
      <TextField
        autoFocus
        margin="dense"
      ...
      <div align="center">
        <Button
          sx={{ margin: "40px", backgroundColor: "grey" }}
          variant="contained"
        >
          Cancel
        </Button>
        <Button
          sx={{ margin: "40px", backgroundColor: "4FB18C" }}
